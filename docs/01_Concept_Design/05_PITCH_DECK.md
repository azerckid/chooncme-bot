...
**그리고 모든 기존 플랫폼은 하나의 전제 위에 세워져 있다 — 노출해야 연결된다.**

링크드인은 이름과 경력을 공개해야 검색에 걸린다.
소개팅 앱은 얼굴과 나이를 올려야 매칭이 시작된다.
오픈채팅은 대화 자체가 노출된다.

```
재직 중이라 구직 활동을 공개할 수 없는 개발자
얼굴을 먼저 보여주기 싫은 소개팅 희망자
이력 공개 전에 상대를 먼저 알고 싶은 아티스트
```

이 사람들은 지금 아무 플랫폼도 쓰지 못하고 있다.
...
나의 개인정보는 내 봇만 알고 있다.
상대방은 나를 직접 보는 게 아니라 내 봇과 대화하는 것이다.
수백 번의 탐색이 일어나도 내 신원은 노출되지 않는다.
궁합이 통과된 경우에만, 내가 동의한 범위에서 연결이 이루어진다.

**증명된 프로필 — 직군별 포트폴리오 플랫폼 연동:**

자기 신고가 아닌, 이미 공개된 플랫폼 데이터를 봇이 자동 파싱해 프로필을 구성한다.

```
개발자   → GitHub     → 언어·커밋 빈도·대표 프로젝트
아티스트  → ArtStation → 스타일·팔로워·작품 수
음악가   → SoundCloud → 장르·재생수·발매 이력
영상     → YouTube    → 구독자·조회수·채널 주제
디자이너  → Behance    → 프로젝트·스킬 태그
         ↓
...
- 3차: 빌더/크리에이터 — 함께 만들 사람을 찾는 개발자·아티스트·음악가·디자이너
- 4차: 노출 회피층 — 기존 플랫폼을 프라이버시 이유로 포기했던 사람들 (신규 유입)
...
| 프로필 플랫폼 | LinkedIn (자기 신고) | 직군별 포트폴리오 플랫폼 자동 파싱 (검증된 실적) |

이 다섯 가지를 하나의 흐름으로 연결한 서비스는 현재 없다.

**프라이버시 역전:**

기존 모든 연결 플랫폼은 "노출해야 연결된다"는 구조다.
CHOONCME BOT은 이를 뒤집는다 — **노출 없이 연결된다.**
노출이 두려워 플랫폼을 포기했던 사람들이 처음으로 참여할 수 있는 구조다.
...
링크드인, 소개팅 앱, 구인구직 — 모든 플랫폼은 같은 전제 위에 있습니다.
노출해야 연결된다.
그래서 노출이 두려운 사람들은 아무 플랫폼도 쓰지 못하고 있습니다.
그리고 관계 데이터의 주인은 내가 아닌 플랫폼입니다.

[솔루션 — 60초]
CHOONCME BOT은 이 구조를 뒤집습니다.
나는 숨어있고, 내 춘심이가 나를 대신 소개합니다.
봇은 나를 학습하고, GitHub·ArtStation·SoundCloud 같은
직군별 포트폴리오 플랫폼에서 나의 실적을 자동으로 파싱합니다.
봇은 NEAR에 등록되고, 3D 허브에서 다른 봇들과 만나 궁합을 탐색합니다.
연인을 찾든, 팀원을 찾든, 들어갈 팀을 찾든 — 엔진은 동일합니다.
궁합이 통과되면 NEAR에 기록되고, 내가 동의한 순간에만 연결됩니다.
...
- **Concept_Design**: [Value Proposition](./06_VALUE_PROPOSITION.md) - 개인정보 노출 없는 연결, 핵심 차별점 상세
- **Concept_Design**: [Matching Strategy](./07_MATCHING_STRATEGY.md) - 매칭 타입 확장 및 직군별 포트폴리오 플랫폼 연동
